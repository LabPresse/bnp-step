...
def sample_b(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, f_vec, eta_vec, nu_vec, gamma, rng,
             temp,kernel):
    """
...
def sample_fh(weak_limit, num_data, data_points, data_times, b_m_vec, t_m_vec, eta_vec, nu_vec, psi, chi, f_ref, h_ref, rng,
              temp,kernel):
    """
...
def sample_t(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
             t_m_vec, f_vec, eta_vec, nu_vec, rng, temp,kernel):
    """
    ...
    for i in range(weak_limit):
        ...
        # Otherwise, perform a Metropolis update
            ...
            bht_matrix = np.multiply(bh_matrix, kernel(((obs_time_matrix - times_matrix))))
            bht_sum = f_vec[-1] + np.sum(bht_matrix, axis=1)
            ...
            log_acceptance_ratio = (exponent_prop - exponent_old) / temp
...
def sample_eta_metropolis(eta_vec, nu_vec, weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, f_vec,
               phi, eta_ref, rng, temp, kernel):
    """
    Samples eta as part of BNP-Step's Gibbs sampler.
...
def calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    ...
    f_vec -- Previous F_bg samples
    eta_vec -- Previous eta samples
    ## MAX: Modify this to take eta_vec[-1] and modify it with point-wise variances
    ...
    new_log_likelihood = np.sum(eta_nu * np.log(eta_nu / (2 * np.pi)) / 2) - exponent_term

    return new_log_likelihood


def calculate_logposterior(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
                           t_m_vec, f_vec, eta_vec, nu_vec, chi, h_ref, gamma, phi, eta_ref, psi, f_ref, kernel):
    """
