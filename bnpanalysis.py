...
#TODO: Modify step plotter to display the appropriate kernel
...
def remove_burn_in(b_vec, h_vec, t_vec, dt_vec, f_vec, eta_vec, post_vec, n):
    """
    ...
    dt_vec_clean = np.delete(dt_vec, np.s_[0:n], 0)
...
def find_map(b_vec, h_vec, t_vec, dt_vec, f_vec, eta_vec, posteriors):
    """
    ...
    dt_clean  = np.asarray(dt_vec[int(map_index)])
...
#TODO: make this work for either step kernel
def generate_step_plot_data(b_vec, h_vec, t_vec, dt_vec, f_vec, weak_limit, t_n, kernel):
    """
    ...
    times_matrix = np.broadcast_to(sampled_times, (num_data, jmp_count))
    obs_time_matrix = np.broadcast_to(t_n, (jmp_count, num_data)).T
    height_matrix = np.broadcast_to(sampled_heights, (num_data, jmp_count))
    load_matrix = np.broadcast_to(sampled_loads, (num_data, jmp_count))
    ...
    # Calculate kernel terms times loads and heights
    if np.size(dt_vec) == 0:
        bht_matrix = np.multiply(bh_matrix,
                                kernel(((obs_time_matrix - times_matrix))))
    else:
        bht_matrix = np.multiply(bh_matrix,
                                kernel(((obs_time_matrix - times_matrix)),dt_vec))
...
def get_step_plot_data(b_m_vec, h_m_vec, t_m_vec, dt_vec, f_vec, data_times, weak_limit, num_data, map_index, kernel):
    """
    ...
    if np.size(dt_vec) == 0:
        ...
        bht_matrix = np.multiply(bh_matrix,
                                kernel(((obs_time_matrix - times_matrix)),dt_vec[map_index]))
    # Calculate sum term for the exponent
    bht_sum = f_vec[map_index] + np.sum(bht_matrix, axis=1)
    return bht_sum

# def generate_step_plot_data(b_vec, h_vec, t_vec, f_vec, weak_limit, t_n, kernel):
#     """
#     Generates a plottable trajectory from a MAP estimate sample from BNP-Step.
...
def generate_gt_step_plot_data(ground_b_m, ground_h_m, ground_t_m, ground_f, data_times, weak_limit,kernel):
    """
...
def generate_histogram_data(b_vec, h_vec, t_vec, num_samples, weak_limit, times):
    ...
    histogram_heights = np.asarray(histogram_heights)
...
def generate_histogram_data_emission(b_vec, h_vec, t_vec, f_vec, weak_limit,kernel):
    """
