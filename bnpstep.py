...
# MAX: chi is underflowing to 0?? I think this is causing problems... let's multiply it by 10...
class BNPStep:
    def __init__(self, 
                 chi: float = 0.028, 
                 dt_ref: float = 100.0,
        ...
        ## MAX: I should just need to add another argument to the constructor here
        Arguments:
        ...
        if not isinstance(dt_ref, float):
            raise TypeError(f"h_ref should be of type float instead of {type(dt_ref)}")
        self.dt_ref = dt_ref
    ...
    def load_data(self, 
        ...
        Arguments:
        ## MAX: I can use the data_format or data_type field to switch on/off the changes later. For now, I'll just leave comments everywhere I change things
        ## MAX: Here, I'll add the loading of nu_vec
        ...
        if not isinstance(kernel_type, str):
            ...
            print(type(kernel_type))
            print(kernel_type)
            self.dataset["kernel_type"] = kernel_type

        if not isinstance(sample_eta, bool):
            raise TypeError(f"sample_eta should be of type bool instead of {type(kernel_type)}")
        else:
            print(type(sample_eta))
            print(sample_eta)
            self.dataset["sample_eta"] = sample_eta


        # if kernel_type == 'heaviside':
        ...
        #     else:
        #         raise ValueError(f"kernel should have value heaviside or ramp instead of {list(kernel_type)}")
    ...
    def analyze(self,
        ...
        #TODO: add value error here
        if self.dataset["kernel_type"] == 'heaviside':
            kernel_func = heav_kernel
        else:
            if self.dataset["kernel_type"] == 'step':
                kernel_func = step_kernel
        ...
        if data["nu_vec"] is not None:
            data["nu_vec"] = data["nu_vec"]
        ...
        # Initialize eta
        if self.dataset["sample_eta"]:
            self.ETA = np.asarray([self.rng.gamma(self.eta_ref / self.phi, self.phi)])
        self.DT  = np.asarray([self.rng.gamma(self.dt_ref / self.chi, self.chi)])

        self.nu_vec = data["nu_vec"]
        # Generate baseline posterior
        poster,likely = sampler.calculate_logposterior(self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
                                                self.T_M, self.DT, self.F_S, self.ETA, self.nu_vec, self.chi, self.h_ref, self.gamma, 
                                                self.phi, self.eta_ref, self.psi, self.dt_ref, self.F_ref, kernel_func)
        self.post = np.asarray([poster])
        self.like = np.asarray([likely])

        # Set temperature for simulated annealing if we are using it
        temperature = 1
        # Gibbs sampler, random sweep
        # for i in np.random.permutation(range(5)):
            # if (i == 0):
            #     # Sample F and h_m
            #     Fh_tmp = sampler.sample_fh(self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.T_M, self.DT, 
            #                                    self.ETA, self.nu_vec, self.psi, self.chi, self.F_ref, self.h_ref, self.rng, 
            #                                    temperature,kernel_func)
            #     h_new = Fh_tmp[1:]
            #     # self.F_S = np.concatenate((self.F_S, np.asarray([Fh_tmp[0]])), axis=0)
            #     self.H_M = np.vstack((self.H_M, h_new))
            #     self.F_S = np.concatenate((self.F_S, [0.0]), axis=0)
            #     self.H_M[0:2] = 10 
            # elif (i == 1):
            #     # Sample b_m
            #     b_new = sampler.sample_b(self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
            #                                  self.T_M, self.DT, self.F_S, self.ETA, self.nu_vec, self.gamma, self.rng, temperature, kernel_func)
            #     self.B_M = np.vstack((self.B_M, b_new))
            # elif (i == 2):
            #     # Sample t_m
            #     t_new = sampler.sample_t(self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
            #                                  self.T_M, self.DT, self.F_S, self.ETA, self.nu_vec, self.rng, temperature, kernel_func)
            #     self.T_M = np.vstack((self.T_M, t_new))
            # elif (i == 3):
            #     # Sample eta
            #     new_eta = sampler.sample_eta_metropolis(self.ETA, self.nu_vec, self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
            #                                      self.T_M, self.DT, self.F_S, self.phi, self.eta_ref, self.rng, temperature, kernel_func)
            #     self.ETA = np.concatenate((self.ETA, np.asarray([new_eta])), axis=0)
            # elif(i == 4):
            #     new_dt = 20 #
            #     # new_dt = sampler.sample_dt_metropolis(self.ETA, self.nu_vec, self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
            #                                 # self.T_M, self.DT, self.F_S, self.phi, self.dt_ref, self.rng, temperature, kernel_func)
            #     self.DT = np.concatenate((self.DT, np.asarray([new_dt])), axis=0)
        ...
        for samp in range(num_samples):
            ...
            for i in np.random.permutation(range(5)):
                if (i == 0):
                    ...
                    Fh_tmp = sampler.sample_fh(self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.T_M, self.DT, 
                                               self.ETA, self.nu_vec, self.psi, self.chi, self.F_ref, self.h_ref, self.rng, 
                                               temperature,kernel_func)
                    # Fh_tmp = np.insert(self.H_M[-1],0,self.F_S[-1]) 
                    ...
                    # Sample eta
                    if not(np.size(self.ETA) == 0):
                        new_eta = sampler.sample_eta_metropolis(self.ETA, self.nu_vec, self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
                                                     self.T_M, self.DT, self.F_S, self.phi, self.eta_ref, self.rng, temperature, kernel_func)
                        # new_eta = 100#self.ETA[-1]
                        self.ETA = np.concatenate((self.ETA, np.asarray([new_eta])), axis=0)
                elif(i == 4):
                    # new_dt = 20 #
                    new_dt = sampler.sample_dt_metropolis(self.ETA, self.nu_vec, self.B_max, len(data["data"]), data["data"], t_n, self.B_M, self.H_M, 
            ...
            self.like = np.concatenate((self.like, np.asarray([likely])), axis=0)
    ...
    def results_to_file(self,
        ...
        # Output data to file
        results = {"b_m": self.B_M, "h_m": self.H_M, "t_m": self.T_M, "dt": self.DT, "F_bg": self.F_S, "eta": self.ETA, "posterior": self.post}
    ...
    def visualize_results(self,
        ...
        burn_in_samples = int(0.01 * len(self.post))
        b_clean, h_clean, t_clean, dt_clean, f_clean, eta_clean, post_clean = bnpa.remove_burn_in(self.B_M, self.H_M, self.T_M, self.DT, self.F_S, self.ETA, self.post, burn_in_samples)
        ...
        for plot in plot_type:
            if (plot == 'step'):
                ...
                for map_index in map_index_list:
                    step_data                 = bnpa.get_step_plot_data(b_clean , h_clean , t_clean , dt_clean, f_clean, t_n , self.B_max , t_n.size , map_index, kernel_func)
                ...
                output_filename = fig_savename + '_' + plot + '.pdf'
                fig.savefig(output_filename, format='pdf')
            elif (plot == 'hist_step_height_abs'):
                # General figure setup
                fig = plt.figure()
                ...
                # TODO: let the user choose how many samples go into the histograms.
                hist_heights, _ = bnpa.generate_histogram_data(good_b_m, good_h_m, good_t_m, len(good_eta), self.B_max, t_n)
                hist_heights    = np.absolute(hist_heights)
                # Plotting - todo: set bins=fixed_bins
                n, bins, patches = ax1.hist(hist_heights, alpha=0.5, edgecolor=learncolor, color=learncolor, density=True, histtype='stepfilled')
                ...
                _, sample_data = bnpa.generate_histogram_data_emission(b_m_top, h_m_top, t_m_top, f_back_top, self.B_max,kernel_func)
                        ...
                        if alt_results[0] == 'ihmm':
                            ...
                            if (self.dataset["ground_truths"] is not None) and (self.dataset["parameters"] is not None):
                                if self.dataset["parameters"]["type"] == 'hmm':
                                    ...
                                    _, ground_data = bnpa.generate_gt_step_plot_data(self.dataset["ground_truths"]["b_m"], self.dataset["ground_truths"]["h_m"], self.dataset["ground_truths"]["t_m"], self.dataset["parameters"]["f_back"], t_n, self.B_max,kernel_func)
