...
def sample_b(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, dt_vec, f_vec, eta_vec, nu_vec, gamma, rng,
             temp,kernel):
    """
    ...
    if np.size(eta_vec) == 0:
            eta_nu      =    nu_vec
    else:
            eta_nu      =    (eta_vec[-1] * nu_vec) / (eta_vec[-1] + nu_vec)
    # Pre-calculate Heaviside terms times associated heights
    
    if np.size(dt_vec) == 0:
        height_heaviside_mat = np.multiply(height_matrix,
                        kernel(((obs_time_matrix - times_matrix))))
    else:
        height_heaviside_mat = np.multiply(height_matrix,
                        kernel(((obs_time_matrix - times_matrix)),dt_vec[-1]))
    ...
    for i in range(weak_limit):
        ...
        exponent_on = np.sum(-(eta_nu / 2) * np.square(data_points - bht_sum))

        # Calculate exponent value for b_m = 0
        load_matrix[:, sampling_order[i]] = 0
        bht_matrix = np.multiply(load_matrix, height_heaviside_mat)
        bht_sum = f_vec[-1] + np.sum(bht_matrix, axis=1)
        exponent_off = np.sum(-(eta_nu / 2) * np.square(data_points - bht_sum))
...
def sample_fh(weak_limit, num_data, data_points, data_times, b_m_vec, t_m_vec, dt_vec, eta_vec, nu_vec, psi, chi, f_ref, h_ref, rng,
              temp,kernel):
    """
    ...
    if np.size(eta_vec) == 0:
        ...
        eta_nu      =    (eta_vec[-1] * nu_vec) / (eta_vec[-1] + nu_vec)
    eta_nu      = eta_nu.reshape(-1,1)
    # Pre-calculate Heaviside terms multiplied by associated loads at each 
    if np.size(dt_vec) == 0:
    ...
    ## MAX: Modify precision and q matrices to accomodate precisions
    ## First, I need to detemine precisely what is going on here...
    # Calculate precision matrix
    ## The precision matrix is initialized by the outer product of a vector of b_n*h_n
    precision_matrix = np.matmul(load_heaviside_mat.T, eta_nu*load_heaviside_mat) + (
            chi * np.eye(num_on_loads, num_on_loads))
    precision_first_row = np.sum(eta_nu*load_heaviside_mat, axis=0)
    ...
    precision_matrix[0, 0] = np.sum(eta_nu) + psi

    # Calculate q matrix - see SI for description of what this is
    q_matrix = (np.matmul(np.squeeze(eta_nu) * data_points, load_heaviside_mat)) + (chi * h_ref)
    q_matrix = np.insert(q_matrix, 0, ( np.sum(np.squeeze(eta_nu) *data_points) + (psi * f_ref)))
    ...
    # new_fh = np.hstack(( np.ravel(MultivariateGaussian.sample(mean_vector, sigma=covariance, epsilon=0.00000001)) ,\
    #                           rng.normal(h_ref, np.sqrt(1 / chi),weak_limit - np.sum(b_m_vec)) ))
...
def sample_t(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
             t_m_vec, dt_vec, f_vec, eta_vec, nu_vec, rng, temp, kernel):
    """
        ...
        # Otherwise, perform a Metropolis update
            ...
            if np.size(dt_vec) == 0:
                ...
                bht_matrix = np.multiply(bh_matrix,
                                kernel(((obs_time_matrix - times_matrix)),dt_vec[-1]))
            bht_sum = f_vec[-1] + np.sum(bht_matrix, axis=1)
            ## MAX: modify to accomodate datapoint-wise variance
            exponent_old = np.sum(-(eta_nu / 2) * np.square(data_points - bht_sum))
            ...
            bht_sum = f_vec[-1] + np.sum(bht_matrix, axis=1)
            ## MAX: modify to accomodate datapoint-wise variance
            exponent_prop = np.sum(-(eta_nu / 2) * np.square(data_points - bht_sum))

            # Calculate acceptance ratio
            # MAX: The acceptance ratio was being computed in real space. Move to log space to prevent underflow
            ...
            if np.log(u_value[i]) < log_acceptance_ratio:
                times_matrix[:, sampling_order[i]] = t_old

    return times_matrix[0]


# def sample_eta(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, dt_vec, f_vec,
...
#     MAX: Update for new eta sampling
#     Returns:
...
def sample_eta_metropolis(eta_vec, nu_vec, weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, dt_vec, f_vec,
               phi, eta_ref, rng, temp, kernel):
    """
    Samples eta as part of BNP-Step's Gibbs sampler.
    ...
    new_eta = eta_vec[-1]
    # pro_eta = rng.gamma(eta_vec[-1]/eta_ref, eta_ref)
    # pro_eta   = rng.gamma(eta_ref / phi, phi)
    pro_eta   = eta_vec[-1] +  1 * rng.normal()
    if (pro_eta > 0):
        acceptance_ratio =  calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
                                                t_m_vec, dt_vec, f_vec, [pro_eta], nu_vec, kernel) + \
                                                ((phi - 1) * np.log(pro_eta)) - ((phi * pro_eta) / eta_ref) - \
                            (calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
                                                ...
                                                ((phi - 1) * np.log(eta_vec[-1])) - ((phi * eta_vec[-1]) / eta_ref))
        
        if (np.log(rng.random()) < acceptance_ratio):
            new_eta = pro_eta
    return new_eta

def sample_dt_metropolis(eta_vec, nu_vec, weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec, t_m_vec, dt_vec, f_vec,
    ...
    new_eta -- the new sample for eta
    """
    # pro_dt = dt_vec[-1] +  0.1 * rng.normal()  #rng.gamma(1/chi, dt_ref/chi)
    # acceptance_ratio =  calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    #                                          t_m_vec, [pro_dt], f_vec, eta_vec, nu_vec, kernel) + \
    #                                 (((chi - 1) * np.log(pro_dt)) - ((chi * pro_dt) / dt_ref) - np.log(sp.special.gamma(chi)) - \
    #                                 (chi * np.log(dt_ref / chi))) - \
    #                     calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    #                                          t_m_vec, dt_vec, f_vec, eta_vec, nu_vec, kernel) - \
    #                                 (((chi - 1) * np.log(dt_vec[-1])) - ((chi * dt_vec[-1]) / dt_ref) - np.log(sp.special.gamma(chi)) - \
    #                                 (chi * np.log(dt_ref / chi)))
    
    # pro_dt =  rng.gamma(1/chi, dt_ref/chi)
    pro_dt = dt_vec[-1] +  1 * rng.normal()
    acceptance_ratio =  calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
                                             t_m_vec, [pro_dt], f_vec, eta_vec, nu_vec, kernel) + \
                        ((chi - 1) * np.log(pro_dt)) - ((chi * pro_dt) / dt_ref) - np.log(sp.special.gamma(chi)) - \
                        (calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    ...
    if (np.log(rng.random()) < acceptance_ratio) & (pro_dt > 0):
        new_dt = pro_dt
    else:
        new_dt = dt_vec[-1]
    return new_dt
...
def calculate_loglikelihood(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    ...
    bht_sum = f_vec[-1] + np.sum(bht_matrix, axis=1)
    exponent_term = np.sum((eta_nu / 2) * np.square(data_points - bht_sum))
    # Calculate log likelihood
    # TODO: check fix, and elsewhere
    new_log_likelihood = np.sum((np.log(eta_nu / (2 * np.pi))) / 2) - exponent_term
...
def calculate_logposterior(weak_limit, num_data, data_points, data_times, b_m_vec, h_m_vec,
    ...
    prior_dt  = ((chi - 1) * np.log(dt_vec[-1])) - ((chi * dt_vec[-1]) / dt_ref) - np.log(sp.special.gamma(chi)) - \
                (chi * np.log(dt_ref / chi))
